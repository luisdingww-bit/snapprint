...
## [0.8.0] - 2026-07-25 · 补回内置模型实例库

> 把 v0.5 浏览器版的「内置模型库」移植到后端，作为社区的第三种内容来源
>（上传 / 照片生成 / 模型实例）。

### 新增
- `app/shapes.py`：16 款参数化真 3D 几何（花瓶、螺旋花瓶、宝石、球体、蛋形、
  圆环、国际象棋兵、陀螺、高脚杯、碗、花盆、蘑菇、保龄球瓶、圣诞树、中式灯笼、
  甜甜圈），移植自浏览器版 `shapes3d.js` 的旋转体 / 圆环算法，带顶点色，
  全部水密可直接切片，仅依赖 numpy + trimesh。
- `GET /api/shapes`：实例库元数据列表。
- `POST /api/shapes/{id}/generate`：一键生成（可调 H/D/seg/twist/lobes，参数
  安全钳制）→ 可打印性分析 → 发布到社区画廊；落盘 .stl（通用）+ .obj（带色）。
- 前端「模型实例」面板：图标网格点选 + 参数微调 + 一键生成，复用报告渲染。
- `tests/test_shapes.py`：16 款全水密 / 参数钳制 / API 全链路，19 测试全过。
